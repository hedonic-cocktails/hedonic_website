...
const pillars = [
  ...
  },
  {
    ...
    description: "Fresh-squeezed citrus, house-made syrups, real fruit — never concentrates, never artificial flavoring. What goes into the bottle is what you'd find behind the finest cocktail bars.",
  },
  {
    icon: Droplets,
    title: "Signature Clarification",
    description: "Our proprietary clarification process — through traditional milk or tropical coconut — transforms each cocktail into crystal-clear liquid with an impossibly silky texture.",
  },
  {
    icon: Wine,
    title: "Living Carbonation",
    description: "Every bottle is carbonated for a crisp, effervescent finish that lifts the flavors and creates a drinking experience unlike anything you've had from a bottle.",
  },
];
  ...
  return (
    <div className="min-h-screen bg-background pt-24 pb-24 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <Link href="/">
            <span className="inline-flex items-center gap-2 font-body text-base text-muted-foreground hover:text-foreground transition-colors cursor-pointer mb-8" data-testid="link-back-why">
        ...
        >
          <Card className="p-8 md:p-12 border-border/30 bg-card/50">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="font-display text-3xl md:text-4xl tracking-wide" data-testid="text-philosophy-title">
                Our Philosophy
              </h2>
            </div>
              ...
              >
                <Card className="p-8 border-border/30 bg-card/50 h-full">
                  <div className="flex items-start gap-5">
                    ...
                    <div>
                      <h3 className="font-display text-2xl tracking-wide mb-3" data-testid={`text-pillar-title-${i}`}>
                        {pillar.title}
                      </h3>
                      <p className="font-body text-base text-muted-foreground leading-relaxed" data-testid={`text-pillar-desc-${i}`}>
                        {pillar.description}
                      </p>
                    </div>
                  </div>
        ...
        <div className="mb-24">
          <motion.div
            ...
            className="text-center mb-16"
          >
            <p className="font-body text-sm tracking-[0.3em] uppercase text-primary mb-4" data-testid="text-compare-label">
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-8 mb-16">
            ...
            >
              <Card className="p-4 md:p-8 border-border/30 bg-card/50 h-full flex flex-col">
                <div className="mb-6">
                  <p className="font-body text-sm tracking-[0.2em] uppercase text-muted-foreground mb-1">Option A</p>
                  <h3 className="font-display text-2xl md:text-3xl tracking-wide" data-testid="text-bar-title">A Night at the Bar</h3>
                </div>
                <div className="space-y-3 mb-6">
                  {barCosts.map((cost, i) => (
                    <div key={i} className="flex items-center justify-between gap-4" data-testid={`row-bar-cost-${i}`}>
                      <div>
                        <p className="font-body text-base text-foreground">{cost.label}</p>
                        <p className="font-body text-sm text-muted-foreground">{cost.detail}</p>
                      </div>
                      <span className="font-body text-base text-foreground">${Math.round(cost.amount)}</span>
                    </div>
                  ))}
                </div>
                <div className="border-t border-border/20 pt-4 mt-auto">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <span className="font-display text-3xl md:text-4xl" data-testid="text-bar-total">${Math.round(barTotal)}</span>
                  </div>
                </div>
              </Card>
            </motion.div>
            ...
            >
              <Card className="p-4 md:p-8 border-primary/30 bg-card/50 h-full flex flex-col relative">
                <div className="absolute -top-3 right-6">
                  <span className="font-body text-sm tracking-[0.15em] uppercase bg-primary text-primary-foreground px-4 py-1.5 rounded-md" data-testid="text-best-value">
                    Best Value
                  </span>
                </div>
                <div className="mb-6">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex items-center justify-between gap-4 text-muted-foreground/40">
                    <p className="font-body text-base line-through">Uber there</p>
                    <span className="font-body text-base line-through">$0</span>
                  </div>
                  <div className="flex items-center justify-between gap-4 text-muted-foreground/40">
                    <p className="font-body text-base line-through">Uber home</p>
                    <span className="font-body text-base line-through">$0</span>
                  </div>
                  <div className="flex items-center justify-between gap-4 text-muted-foreground/40">
                    <p className="font-body text-base line-through">Tip</p>
                    <span className="font-body text-base line-through">$0</span>
                  </div>
                </div>
                <div className="border-t border-primary/20 pt-4 mt-auto">
                  <div className="flex items-center justify-between gap-4">
                    <span className="font-display text-xl">Total</span>
                    <span className="font-display text-3xl md:text-4xl text-primary" data-testid="text-hedonic-total">${Math.round(hedonicTotal)}</span>
                  </div>
                </div>
              </Card>
            </motion.div>
          ...
          >
            <Card className="p-8 md:p-12 border-primary/30 bg-card/50 text-center">
              <p className="font-body text-sm tracking-[0.3em] uppercase text-primary mb-4" data-testid="text-savings-label">
                Your Savings
              </p>
              <h3 className="font-display text-6xl md:text-8xl tracking-wide text-primary mb-2" data-testid="text-savings-pct">
                {savingsPercent}%
              </h3>
            ...
            {headToHead.map((item, i) => (
              ...
              >
                <Card className="p-5 border-border/30 bg-card/50 text-center h-full" data-testid={`card-h2h-${i}`}>
                  ...
                  <p className="font-display text-base tracking-wide mb-3">{item.category}</p>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2 text-left">
                      <X className="w-4 h-4 text-destructive flex-shrink-0 mt-0.5" />
                      <span className="font-body text-sm text-muted-foreground leading-relaxed">{item.bar}</span>
                    </div>
                    <div className="flex items-start gap-2 text-left">
                      <Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" />
                      <span className="font-body text-sm text-foreground leading-relaxed">{item.hedonic}</span>
                    </div>
                  </div>
                </Card>
              </motion.div>
        ...
        >
          <h2 className="font-display text-4xl md:text-5xl tracking-wide italic mb-4" data-testid="text-why-cta">
            Ready to Indulge?
          </h2>
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            <Link href="/quiz">
              <Button className="font-body text-base tracking-widest uppercase px-8" data-testid="button-why-quiz">
                Find Your Bottle
                <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            </Link>
            <Link href="/collection">
              <Button variant="outline" className="font-body text-base tracking-widest uppercase px-8" data-testid="button-why-collection">
                Browse All
              </Button>
            </Link>
          </div>

...
// ../.wrangler/tmp/bundle-WrBeAx/checked-fetch.js
...
var products = sqliteTable("products", {
  ...
  imageUrls: text("image_urls", { mode: "json" }).$type().notNull().default(sql`'[]'`),
...
var insertBatchSchema = createInsertSchema(batches).omit({ id: true, createdAt: true });

// api/inventory/recipes.ts
...
// api/inventory/[id].ts
function checkAuth2(request, env) {
  const pass = request.headers.get("x-admin-password");
  const expected = env.INVENTORY_PASSWORD || "licitadmin";
  return pass === expected;
}
...
var onRequestPatch = /* @__PURE__ */ __name(async (context) => {
  if (!checkAuth2(context.request, context.env)) return new Response("Unauthorized", { status: 401 });
...
function checkAuth3(request, env) {
  const pass = request.headers.get("x-admin-password");
  const expected = env.INVENTORY_PASSWORD || "licitadmin";
  return pass === expected;
}
...
var onRequestGet = /* @__PURE__ */ __name(async (context) => {
  if (!checkAuth3(context.request, context.env)) {
...
// ../.wrangler/tmp/pages-lVWx2X/functionsRoutes-0.32777823851624266.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-WrBeAx/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-WrBeAx/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6175300318041275.mjs.map
